...
seg_cw_NE <- ncdb_cw %>%
  ...
  filter(!(is.na(MSA70) & is.na(MSA80) & is.na(MSA90) & is.na(MSA00))) %>% #is not all NAs for historical (micropolitan)
  filter(NEWENG) %>%
...
cbsa_sum <- seg_D %>%
  ...
  mutate(prop70 = nMSA70/nCBSA,
         prop80 = nMSA80/nCBSA,
         prop90 = nMSA90/nCBSA,
         prop00 = nMSA00/nCBSA) #if county is not associated with metro area code through 70-00, must be micropolitan MSA cty added in 2010
